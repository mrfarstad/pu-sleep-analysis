...
public class DoctorGUITest extends ApplicationTest {
  ...
  public void signIn() {
    ...
    when(Login.userDataFetch.signIn(username, password)).thenReturn(doctorSample);
    when(Login.userDataFetch.getCurrentUser()).thenReturn(doctorSample);
  ...
  public void testRequestButton() {
    ...
    DataAccessRequest request =
        new DataAccessRequest(
            "requestID", userSample, Login.userDataFetch.getCurrentUser(), "PENDING");
    when(Login.userDataFetch.getUserByUsername(username)).thenReturn(userSample);
